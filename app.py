...
def index():
    if "login" in session and session["login"]:
        ...
        return render_template("home.html", message=msg, msg4=msg2, msg2=msg2)
    else:
...
def login():
    return render_template(
        "login.html", title="Login", err="False", message="IDとパスワードを入力:", id=""
    )
...
def login_post():
    ...
    if session["login"]:
        ...
        return render_template(
            "login.html", title="Login", err=False, message="idまたはpasswordが違います。", id=id
        )
...
def setting():
    if "login" in session and session["login"]:
        ...
        for i in range(0, len(msg2)):
            msg2[i][4] = "class/" + class_list[i][4]
        return render_template(
            "setting.html", message=msg, msg3=msg3, err="1", msg4=msg2
        )
    else:
        return redirect("/login")
    ...
    if "login" in session and session["login"]:
        ...
        if not key == "None":
            ...
            for i in range(0, len(msg2)):
                msg2[i][4] = "class/" + class_list[i][4]
            return render_template(
                "setting.html", message=msg, msg3=msg3, err=err, msg4=msg2
            )
        else:
            return "/logout"
    else:
                ...
                if session["license"]:
                            ...
                            if not class_list[i][6] == "0":
                                ...
                                for i in range(0, len(status_list)):
                                    if status_list[i][5] == None:
                                        continue
                                    elif float(status_list[i][5]) <= float(
                                        current - 1800
                                    ):
                    ...
                    if len(form_list) == 0:
                        ...
                        for i in range(0, len(form_list)):
                            ...
                            newform_list[i].append(
                                color_list[random.randrange(len(color_list))]
                            )
        ...
        if "license" in session:
            if session["license"] == True:
                ...
                if class_name in url_list:
                    ...
                    cur.execute(
                        'UPDATE classes SET status="1" WHERE url="' + class_name + '"'
                    )
                    ...
                    cur.execute(
                        'CREATE TABLE "'
                        + class_name
                        + '_form" ("id"	INTEGER NOT NULL UNIQUE,"url"	NUMERIC NOT NULL UNIQUE,"sentence"	TEXT NOT NULL,"style"	TEXT NOT NULL,PRIMARY KEY("id" AUTOINCREMENT))'
                    )
                    conn.commit()
                    cur.close()
                    conn.close()
                        ...
                        if class_list[i][4] == class_name:
                            if not class_list[i][6] == "0":
                                ...
                                cur.execute(
                                    "UPDATE "
                                    + class_name
                                    + ' SET status=REPLACE(status,"1","5")'
                                )
                                cur.execute(
                                    "UPDATE "
                                    + class_name
                                    + ' SET status=REPLACE(status,"2","5")'
                                )
                                cur.execute(
                                    "UPDATE "
                                    + class_name
                                    + ' SET status=REPLACE(status,"3","5")'
                                )
                                cur.execute(
                                    "UPDATE "
                                    + class_name
                                    + ' SET status=REPLACE(status,"4","5")'
                                )
                                cur.execute(
                                    'UPDATE classes SET status="2" WHERE url="'
                                    + class_name
                                    + '"'
                                )
                                conn.commit()
